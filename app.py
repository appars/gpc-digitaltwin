...
from flask import Flask, render_template, redirect, url_for, Response, send_from_directory, request, jsonify
...
wgc_state = {
    ...
    "health": {"vib_axial": 2.2, "vib_vert": 2.8, "vib_horz": 2.5, "bearing_temp": 345.0, "oil_temp": 325.0,
               "lube_oil_pressure": 3.2, "seal_leakage": 0.12},
    "kpi": {}
}
...
@app.route("/wgc")
def wgc_index():
    app.logger.info("[HTTP] render /wgc")
...
def wgc_history_csv():
    ...
    w.writerow(["timestamp","flow","P1","P2","T1","T2","speed","valve","vib_axial","vib_vert","vib_horz",
                "lube_oil_pressure","bearing_temp","oil_temp","seal_leakage","compression_ratio",
                "surge_margin_pct","head_index_norm","efficiency_index"])
    for r in wgc_history:
        w.writerow([r["ts"], r["flow"], r["P1"], r["P2"], r["T1"], r["T2"], r["speed"], r["valve"], r["v_ax"], r["v_v"],
                    r["v_h"], r["oil_p"], r["bearingT"], r["oilT"], r["seal"], r["cr"], r["sm_pct"], r["head_norm"], r["eff_idx"]])
...
def wgc_ingest():
    ...
    app.logger.info("[HTTP] ingest keys=%s", list(data.keys()))
...
@socketio.on("connect")
def on_connect():
    app.logger.info("[WS] dashboard connected")
    socketio.emit("update_wgc", wgc_state)

@socketio.on("wgc_data")
def wgc_data(data):
    app.logger.info("[WS] wgc_data keys=%s", list(data.keys()))
    for section in ("gas","oper","health"):
        if section in data:
            wgc_state.setdefault(section, {}).update(data[section])
    wgc_state["kpi"] = compute_wgc_kpis(wgc_state)
...
def wgc_command(data):
    ...
    app.logger.info("[WS] wgc_command %s", action)
